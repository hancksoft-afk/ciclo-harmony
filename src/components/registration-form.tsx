...
export function RegistrationForm() {
  ...
  const validateStep1 = () => {
    ...
    if (!formData.invitee || formData.invitee.split(' ').length < 2) {
      newErrors.invitee = true;
    }
    if (!formData.email || !formData.email.includes('@')) {
      newErrors.email = true;
    }
    if (formData.paymentMethod === 'binance_pay' && (!formData.binanceId || formData.binanceId.length < 9)) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  {errors.name && (
                    <div className="mt-1.5 text-xs text-amber-300 flex items-center gap-1.5">
                      ...
                      <span><strong className="font-medium">Nombre inválido</strong> — Debe tener al menos 2 palabras.</span>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.invitee && (
                    <div className="mt-1.5 text-xs text-amber-300 flex items-center gap-1.5">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span><strong className="font-medium">Invitado inválido</strong> — Debe tener al menos 2 palabras.</span>
                    </div>
                  )}
                </div>
              ...
              {formData.paymentMethod === 'binance_pay' && (
                <div>
                  ...
                  {errors.binanceId && (
                    <div className="mt-1.5 text-xs text-amber-300 flex items-center gap-1.5">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      <span><strong className="font-medium">ID de Binance inválido</strong> — Debe tener 9 o 10 dígitos numéricos.</span>
                    </div>
                  )}
                </div>
              )}
